package com.cyberwalkabout.common.expandableadapter;

import android.view.View;
import android.widget.ListAdapter;
import com.cyberwalkabout.common.R;

/**
 * ListAdapter that adds sliding functionality to a list.
 * Uses R.id.expandalbe_toggle_button and R.id.expandable id's if no
 * ids are given in the contructor.
 *
 * @author tjerk
 * @date 6/13/12 8:04 AM
 */
public class SlideExpandableListAdapter extends AbstractSlideExpandableListAdapter {
	private int expand_button_id;
    private int collapse_button_id;
	private int expandable_view_id;

	public SlideExpandableListAdapter(ListAdapter wrapped, int expand_button_id, int expandable_view_id, int collapse_button_id) {
		super(wrapped);
		this.expand_button_id = expand_button_id;
		this.expandable_view_id = expandable_view_id;
        this.collapse_button_id = collapse_button_id;
	}

	public SlideExpandableListAdapter(ListAdapter wrapped) {
		this(wrapped, R.id.expandable_toggle_button, R.id.expandable, R.id.collapse_toggle_button);
	}

	@Override
	public View getExpandToggleButton(View parent) {
		return parent.findViewById(expand_button_id);
	}

	@Override
	public View getExpandableView(View parent) {
		return parent.findViewById(expandable_view_id);
	}

    @Override
    public View getCollapseToggleButton(View parent) {
        return parent.findViewById(collapse_button_id);
    }
}
